...
type AppRole = "admin" | "customer" | "contractor";
...
interface AuthContextType {
  ...
  loading: boolean;
  role: AppRole | null;
  profile: { full_name: string; mobile: string; email: string; profile_picture_url: string } | null;
  ...
  const fetchUserData = async (userId: string) => {
    try {
      const [rolesRes, profileRes] = await Promise.all([
        ...
        supabase.from("profiles").select("full_name, mobile, email, profile_picture_url").eq("user_id", userId).maybeSingle(),
      ]);
